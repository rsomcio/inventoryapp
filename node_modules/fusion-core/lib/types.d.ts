...
export type Middleware = (ctx: Context, next: () => Promise<void>) => Promise<any>;
export type MiddlewareWithDeps<Deps extends FusionPluginDepsType> = (Deps: ExtractDepsType<Deps>) => Middleware;
...
export type ExtractTokenType<T extends Token<any>> = T extends Token<infer R> ? R : never;
export type ExtractServiceType<P> = P extends FusionPlugin<infer D, infer S> ? S : P;
