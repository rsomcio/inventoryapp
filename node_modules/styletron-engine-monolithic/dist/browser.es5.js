...
function injectStylePrefixed(styles, selector, prefix$$1, strict) {
  ...
  for (var key in styles) {
    ...
    if (_typeof(value) !== "object") {
      ...
      var rule = hyphenateStyleName(key) + ":" + value;
      var prefixed = inlineStylePrefixer.prefix((_prefixRule = {}, _prefixRule[key] = value, _prefixRule));
      ...
      for (var prefixedKey in prefixed) {
        ...
        if (typeof prefixedVal === "string" || typeof prefixedVal === "number") {
          var prefixedRule = hyphenateStyleName(prefixedKey) + ":" + prefixedVal;
          ...
          for (var i = 0; i < prefixedVal.length; i++) {
            var _prefixedRule = hyphenated + ":" + prefixedVal[i];

            if (_prefixedRule !== rule) {
              rules += _prefixedRule + ";";
            }
          }
        }
    ...
    if (key[0] === ":") {
      classes = [].concat(classes, injectStylePrefixed(value, selector + key, prefix$$1, strict));
      continue;
    }

    if (key[0] === "@") {
      var nestedRules = injectStylePrefixed(value, selector, prefix$$1, strict);
      classes = [].concat(classes, [key + "{" + nestedRules.join("") + "}"]);
      continue;
    }

    classes = [].concat(classes, [key + "{" + injectStylePrefixed(value, "", prefix$$1, strict).join("") + "}"]);
...
exports.Client = StyletronClient;
exports.Server = StyletronServer;
